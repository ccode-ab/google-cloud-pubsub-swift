...
public final class PubSubSubscriber {
    ...
    private func pull(from subscription: Subscription, use handler: @escaping Handler) -> EventLoopFuture<Void> {
        print("Pulling...")
        ...
        return driver.raw
            ...
            .flatMap { response in
                ...
                if let allFutures = allFutures {
                    return allFutures.flatMap {
                        self.pull(from: subscription, use: handler)
                    }
                } else {
                    return self.pull(from: subscription, use: handler)
                }
            }
    }

    public func receive(from subscription: Subscription, use handler: @escaping Handler) -> EventLoopFuture<Void> {
